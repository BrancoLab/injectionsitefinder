...
def visualize_injections(
    ...
    fakescene = Scene(
        add_root=add_root
    )  # used to load meshes that should not be rendered
    ...
    if regions:
        scene.add_brain_regions(
            regions, use_original_color=True, **regions_kwargs
        )
...
def render_experiment(
    ...
    if not isinstance(cells, list):
        cells = [cells]
    if not isinstance(injections_colors, list):
        injections_colors = [injections_colors]
    if not isinstance(cells_colors, list):
    ...
    for cellfile, cellcolor in zip(cells, cells_colors):
        scene.add_cells_from_file(
            cellfile, color=cellcolor, radius=10, alpha=0.3
        )
