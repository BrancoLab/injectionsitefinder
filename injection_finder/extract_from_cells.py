...
target_com = scene.get_region_CenterOfMass(
    target_region, unilateral=False
...
cells_in_range = [
    c
    for c, d in zip(cells_coords, cells_com_distances)
    if d <= max_distance_from_target
]
...
hull = convexHull(cells_to_exclude).alpha(0.5).color("salmon")
...
scene.add_brain_regions(
    ["SCm"], use_original_color=True, wireframe=True, alpha=0.5
)
scene.render()
